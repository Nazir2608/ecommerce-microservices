    ...
    public ErrorDecoder errorDecoder() {
        ...
        return (methodKey, response) -> {
            if (response.status() == 404) {
                return new com.nazir.ecommerce.orderservice.exception.ProductUnavailableException("Product not found (product-service returned 404)");
            }
            if (response.status() == 409) {
                return new com.nazir.ecommerce.orderservice.exception.ProductUnavailableException("Insufficient stock (product-service returned 409)");
            }
